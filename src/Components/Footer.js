...
const UserDetails = ({page,setPage,totalNumberOfPages }) =>{
  const pageLimit = 5;
  const [begin,setStart ] = useState(1);
  const [end, setEnd] = useState(Math.min(pageLimit, totalNumberOfPages));

  const getUserDetailsButtons = (begin,end) => {
    return Array.from({ length: (end - begin) / 1+ 1 }, (_, i) => begin + (i * 1));
  };

  const [pageNum, setPageNum] = useState(getUserDetailsButtons(begin, end, 1));

  const checkRange = () => {    
    setPageNum(getUserDetailsButtons(begin,totalNumberOfPages));
  }
